...
# Background thread to listen for incoming UDP packets
class ReceiverThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            if text == "__SYS:FILE_RX_MODE":
                self.sys_command.emit("FILE_RX_MODE")
...
class WaveLinkApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("WaveLink Chat (Windows)")
        self.resize(450, 650)
    ...
    def switch_flowgraph(self, script_name):
        ...
        # WINDOWS SPECIFIC: Prevents black CMD windows from popping up 
        # on the user's screen every time a background subprocess is started.
        kwargs = {}
    ...
    def revert_to_chat(self):
        # 1. We must spin up the local Chat flowgraph first so we have access to the UDP transmission link
        self.switch_flowgraph('bpskrxtx.py')
        
        def delayed_revert_trigger():
            # 2. Give the PlutoSDR 3.5 seconds to fully initialize and lock the hardware
            time.sleep(3.5) 
            padded_cmd = "__SYS:CHAT_MODE".ljust(1024, ' ')
            self.tx_socket.sendto(padded_cmd.encode('utf-8'), ("127.0.0.1", 5004))
            
            # Send a dummy packet to push the command through the buffer
            time.sleep(0.05)
            dummy = " ".ljust(1024, ' ')
    ...
    def send_message(self):
        ...
        if text:
            padded_text = (text + '\n').ljust(1024, ' ')
            self.tx_socket.sendto(padded_text.encode('utf-8'), ("127.0.0.1", 5004))
